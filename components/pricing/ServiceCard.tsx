...
export function ServiceCard({ service, onViewDetails, isBlueCard }: ServiceCardProps) {
        ...
        <Card className={`flex flex-col h-full border-slate-200 transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group ${
            ...
            <CardHeader className="pb-4 pt-8">
                <h3 className={`text-xl font-bold font-figtree leading-tight min-h-[3.5rem] ${
                    isBlueCard || isFinancialCard ? 'text-white' : 'text-[#111A4A]'
                }`}>
                    {service.title}
                </h3>
                <p className={`text-sm min-h-[2.5rem] ${
                    isBlueCard || isFinancialCard ? 'text-white/90' : 'text-slate-500'
                }`}>
                    {service.outcome}
                </p>
            </CardHeader>
            ...
            <CardContent className="flex-grow space-y-6">
                <div>
                    ...
                    <div className={`text-xs font-medium mt-1 ${
                        isBlueCard || isFinancialCard ? 'text-white/80' : 'text-slate-400'
                    }`}>Typical range</div>
                </div>
                ...
                <ul className="space-y-3">
                    {service.bullets.map((bullet, i) => (
                        <li key={i} className={`flex items-start gap-2 text-sm ${
                            isBlueCard || isFinancialCard ? 'text-white/95' : 'text-slate-600'
                        }`}>
                            <Check className={`w-4 h-4 mt-0.5 flex-shrink-0 ${
                                isBlueCard || isFinancialCard ? 'text-white' : 'text-[#156d95]'
                            }`} />
                            <span>{bullet}</span>
                        </li>
                    ))}
